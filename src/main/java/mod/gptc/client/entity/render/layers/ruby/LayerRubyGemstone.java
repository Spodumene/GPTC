...
public class LayerRubyGemstone extends FeatureRenderer<GemRuby, ModelRuby> {
	public LayerRubyGemstone(FeatureRendererContext<GemRuby, ModelRuby> context) {
		super(context);
	...
	public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, GemRuby entity,
			...
			float headPitch) {
        int skin = entity.getGemColor();
        float r = ((skin & 16711680) >> 16) / 255f;
        float g = ((skin & 65280) >> 8) / 255f;
        float b = ((skin & 255) >> 0) / 255f;
        VertexConsumer builder = vertexConsumers.getBuffer(RenderLayer.getEntityCutoutNoCull(new Identifier("gptc:textures/entities/ruby/gems/" + entity.getGemCut().id + "_" + entity.getGemPlacement().id + ".png")));
